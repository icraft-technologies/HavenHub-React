...
const SocialSignIn = () => {
    // Replace these URLs with your Laravel backend routes for social login
    const googleLoginUrl = `${API_BASE_URL}/api/auth/google/redirect`;
    const githubLoginUrl = `${API_BASE_URL}/api/auth/github/redirect`;
    ...
    return (
        <div className="flex gap-4">
            <button
                onClick={handleSocialLogin(googleLoginUrl)}
            ...
            <button
                onClick={handleSocialLogin(githubLoginUrl)}
                className="flex w-full items-center justify-center gap-2.5 rounded-lg border border-border dark:border-dark_border p-3.5 text-dark duration-200 ease-in dark:border-darkborder dark:text-white dark:hover:bg-primary/10 hover:bg-primary/10"
            >
                Sign In
                <svg
